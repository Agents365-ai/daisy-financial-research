...
All mutating scripts accept `--out-dir <root>`; subdirs are appended automatically. `technical_indicators.py` and `debate_runner.py` are read-only outside their input/pad files and have no `--out-dir`.
